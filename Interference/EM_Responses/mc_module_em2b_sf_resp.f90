...
subroutine int_eval(p2,ctp2,phip2,p1,phip1,ip1,ie1,w,r_now)
   ...
   !NR
   !ctp1 = (xmn*q_4(1)/qval  - qval/2.0d0)/p1
   q2=q_4(1)**2-qval**2
...
!.......currents
  ...
  !    dp2=1 !to be used to mediate with a RFG on particle 2
   ...
   jac = pp1_4(1)/(p1*qval)
      ...
      !ONEBODY
      if(i_intf.ne.1)then
         r_now(1,:) = (dble(xA)/rho)*dp1*p1**2*(2.0d0*pi)*onebody(:)*jac  &  
         &     /(2.0d0*pi)**3!/(norm/(dble(xA)/2.0d0)) !*dp2*p2**2*(4.0d0*pi)/(4.0d0*pi*xpf**3/3.0d0)  
! note that since I am using the MF SF I can't compute the 1b contribution in a correct way
      ...
      !TWOBODY
         ...
         r_now(2,:) = (dble(xA)/rho)*dp1*p1**2*(2.0d0*pi)*(-exc(:) + dir(:))*jac  &
         &     /(2.0d0*pi)**3 *dp2*p2**2*(4.0d0*pi*xpmax)/(2.0d0*pi)**3 ! use xpf  to mediate with a RFG on particle 2
      endif
