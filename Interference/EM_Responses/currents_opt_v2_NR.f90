...
subroutine det_J1Jdel_exc(gd,res_re,nuc1_iso)
 ...
 subroutine det_J1Jdel_exc_nr(gd,res_re,nuc1_iso)
   ...
   integer*4 :: nuc1_iso !This is the isospin of the struck nucleon
   complex*16 :: nuc1_isospinor(2)
   complex*16 :: const,pi_prop1,pi_prop2
   real*8 :: k1crossq(3),k2crossq(3),gd
   complex*16 :: J1b(4,2,2), J1c(4,2,2),res(4,4)
   complex*16 :: J1bdag(4,2,2), J1cdag(4,2,2),j1del(4,4)
   real*8 :: res_re(4,4)
   complex*16 :: cta,ctb,ctc,ctd ! isospin coefficients
   ...
   do i1=1,2
      do f1=1,2
         !do i=1,4
            ...
            !J_mu(f1,i1,2)=sig(2,f1,i1)*q(4)*ci*gd/2.0d0/xmn
            !J_mu(f1,i1,3)=-sig(1,f1,i1)*q(4)*ci*gd/2.0d0/xmn
            J_mu(f1,i1,2)=(ci*gd/2.0d0/xmn)*(q(4)*sig(2,f1,i1))
            J_mu(f1,i1,3)=-(ci*gd/2.0d0/xmn)*(q(4)*sig(1,f1,i1))
         !enddo
      enddo
   enddo
   ...
   k1crossq = cross_product(k1(2:4),q(2:4))
   k2crossq = cross_product(k2(2:4),q(2:4))

   const = 4.2d0*(ci/9.0d0)*(1.0d0/xmn)*2.256d0*sqrt(0.08d0*4.0d0*pi/xmpi**4)*1.0d0/(xmd - xmn) 
   !print*,'const = ', const
   !const = 
      ...
      do f1=1,2
        do i2=1,2
          do it2=1,2
            ...
            J1b(i,f1,i1) = J1b(i,f1,i1) + const*pi_prop1*sum(sig(1:3,i2,i1)*k1(2:4))* &
            &   ( 4.0d0*idelta_single1_EM_NR(t2(it2,:),nuc1_isospinor, nuc1_isospinor, t2(it2,:))*k1crossq(i-1) &
            &   + idelta_cross_EM_NR(t2(it2,:),nuc1_isospinor, nuc1_isospinor, t2(it2,:)) &
            &   *( sum(sig(1:3,f1,i2)*q(2:4))*k1(i) - sum(sig(1:3,f1,i2)*k1(2:4))*q(i)) ) 
            
            J1c(i,f1,i1) = J1c(i,f1,i1) + const*pi_prop2*sum(sig(1:3,f1,i2)*k2(2:4))* &
            &   ( 4.0d0*idelta_single2_EM_NR(t2(it2,:),nuc1_isospinor, nuc1_isospinor, t2(it2,:))*k2crossq(i-1) &
            &   - idelta_cross_EM_NR(t2(it2,:),nuc1_isospinor, nuc1_isospinor, t2(it2,:)) &
            &   *( sum(sig(1:3,i2,i1)*q(2:4))*k2(i) - sum(sig(1:3,i2,i1)*k2(2:4))*q(i)) ) 

          enddo
        enddo

        J1bdag(i,f1,i1)=conjg(J1b(i,f1,i1))
        J1cdag(i,f1,i1)=conjg(J1c(i,f1,i1)) 
      enddo
    enddo
   enddo
  ...
  !do j=1,4
     do i1=1,2
        do f1=1,2
           do i2=1,2
              j=i
              j1del(i,j)=j1del(i,j) + J1bdag(i,f1,i1)*J_mu(f1,i1,j) + &
              & J1cdag(i,f1,i1)*J_mu(f1,i1,j)
           enddo
        enddo
     enddo
   !enddo
...
function idelta_cross_EM_NR(it1,it2,itp1,itp2) result(t)
    implicit none
    complex*16 :: it1(2),it2(2),itp1(2),itp2(2)
    complex*16 :: IDeltaDDag_EM, c,d,t

    t = (me(1,it1,itp1)*me(2,it2,itp2) - me(2,it1,itp1)*me(1,it2,itp2))
end function idelta_cross_EM_NR


function idelta_single1_EM_NR(it1,it2,itp1,itp2) result(t)
    implicit none
    complex*16 :: it1(2),it2(2),itp1(2),itp2(2)
    complex*16 :: IDeltaDDag_EM, c,d,t

    t = me(3,it1,itp1)*iden(it2,itp2)

end function idelta_single1_EM_NR

function idelta_single2_EM_NR(it1,it2,itp1,itp2) result(t)
    implicit none
    complex*16 :: it1(2),it2(2),itp1(2),itp2(2)
    complex*16 :: IDeltaDDag_EM, c,d,t

    t = me(3,it2,itp2)*iden(it1,itp1)

end function idelta_single2_EM_NR
